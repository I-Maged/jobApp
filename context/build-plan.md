...
Extract from Resume button — AI reads uploaded PDF and auto-fills profile form fields.
...
- AI reads extracted text and returns structured JSON matching all profile field names
...
Generate a clean professional PDF resume from current profile data using OpenRouter AI.
...
- @react-pdf/renderer renders AI output into clean single-page PDF using renderToBuffer()
...
- For each job returned:
  ...
  - AI scores job against user profile:
...
- AI Match Reasoning section — match reason paragraph written by AI
...
- Always return a dossier — never fail silently. If browser research failed, AI synthesizes from job description and profile alone.
