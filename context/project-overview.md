...
JobPilot is a full stack AI-powered job hunting assistant. The user sets up their profile once, uploads their resume, and the agent automatically discovers relevant jobs from Adzuna — scoring each one against the user's profile using OpenRouter AI. For jobs they're interested in, the agent researches the company across their public web pages and builds a structured dossier — company overview, tech stack, culture, why the role exists, and interview prep. The user reviews everything and applies with one click.
...
- User can generate a clean professional PDF resume from their current profile data using OpenRouter AI
...
- Resume PDF generation from profile data using OpenRouter AI
